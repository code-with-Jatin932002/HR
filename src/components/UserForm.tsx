...
import { FaEye, FaEyeSlash, FaChevronDown } from 'react-icons/fa'; // Added FaChevronDown
import callApi from '@/utils/callApi';
import Button from './Button';

// --- NEW STYLES DEFINITION ---
// Adjusted vertical padding to py-3 to ensure better vertical alignment across all input types
const INPUT_STYLE = `
  w-full rounded-lg border border-gray-300 py-4 px-6 text-gray-900 outline-none transition duration-300
  focus:border-purple-600 focus:ring-2 focus:ring-purple-200
`;
// Style for SELECT fields: use INPUT_STYLE plus appearance-none to normalize height and remove default arrow
const SELECT_STYLE = `${INPUT_STYLE} appearance-none`;

const LABEL_STYLE = 'mb-2.5 block text-sm font-medium text-gray-700';
// --- END NEW STYLES DEFINITION ---
...
export default function UserForm({ isUpdate, initialValues, onCancel, onSubmit, currentUserRole, isSubmitting = false }: UserFormProps) {
  ...
  // Helper component for Select/Dropdowns to ensure consistent height and look
  const SelectWrapper: React.FC<React.PropsWithChildren<{}>> = ({ children }) => (
    <div className="relative">
      {children}
      <FaChevronDown className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
    </div>
  );
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {!isUpdate && (
            <div className="relative">
              ...
              />
              <div
                className="absolute right-3 top-1/2 mt-1.5 transform -translate-y-1/2 cursor-pointer text-gray-600"
          ...
          {/* Joining Date */}
          <div>
            <label className={LABEL_STYLE}>Joining Date</label>
            <input
              type="date"
              name="joining_date"
          ...
          <div>
            ...
            <SelectWrapper>
              <select
                ...
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
                className={SELECT_STYLE} // APPLIED SELECT_STYLE
                disabled={isSubmitting}
              >
          ...
          <div>
            <label className={LABEL_STYLE}>Marital Status</label>
            <SelectWrapper>
              <select
                name="marital_status"
          ...
          <div>
            <label className={LABEL_STYLE}>Employee Type</label>
            <SelectWrapper>
              <select
                name="employee_type"
          ...
          {/* Working Days */}
          <div>
            <label className={LABEL_STYLE}>Week Working Days</label>
            <input
              type="text"
              name="working_days"
          ...
          {/* Official Email */}
          <div>
            <label className={LABEL_STYLE}>Official Email</label>
            <input
              type="email"
              name="official_email"
          ...
          {/* Slack ID */}
          <div>
            <label className={LABEL_STYLE}>Slack ID</label>
            <input
              type="text"
              name="slack_id"
          ...
          {/* GitHub ID */}
          <div>
            <label className={LABEL_STYLE}>GitHub ID</label>
            <input
              type="text"
              name="github_id"
          ...
          {/* Role Type (Select) */}
          <div>
            <label className={LABEL_STYLE}>Role Type</label>
            {loadingRoles ? (
              <p className="py-3">Loading roles...</p>
            ) : rolesError ? (
          ...
          {currentUserRole.toLowerCase() !== 'super_admin' && (selectedRole.toLowerCase() !== 'admin' && selectedRole.toLowerCase() !== 'super_admin') && (
            <div className="col-span-1 md:col-span-2">
              <label className={LABEL_STYLE}>Department Name</label>
              {loadingDepartments ? (
                <p className="py-3">Loading departments...</p>
              ) : departmentError ? (
