...
export default function CandidatePage() {
  ...
  // Determine if the current user is HR or Admin
  const canManageCandidates = userRole === 'hr' || userRole === 'admin';
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8">
      <div className="mx-auto mt-10 max-w-6xl rounded bg-white p-6 shadow">
          ...
          {canManageCandidates && (
            <Button
              ...
              onClick={() => {
                ...
                setSelectedCandidateId('');
                formik.resetForm();
                formik.setFieldValue('status', statusOptions[0]); // Ensure default status is set
              }}
              variant="primary"
        ...
        {formOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4"> {/* Added p-4 for padding on small screens */}
            {/* Added max-h-full, overflow-y-auto to allow scrolling within the modal */}
            ...
            actions={(candidate: Candidate) => (
              <div className="relative">
                <ActionButtons
                  ...
                  onUpdate={canManageCandidates ? () => handleUpdate(candidate) : undefined}
                  onDelete={canManageCandidates ? () => handleDelete(candidate.id) : undefined}
                  showView={true}
                />
