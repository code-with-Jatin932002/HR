...
import { FiX } from 'react-icons/fi'; // <-- Added FiX import
...
  // Fetch rejected leaves data
  const {
    data: leavesData,
    isLoading: isLoadingLeaves,
    isError: isErrorLeaves,
    refetch: refetchLeaves,
  } = useQuery<LeaveApiResponse, Error, LeaveApiResponse, ['rejectedLeaves', number, number]>({
    queryKey: ['rejectedLeaves', currentPage, itemsPerPage],
    queryFn: async ({ queryKey }) => {
      const [_key, page, limit] = queryKey;
      if (!token) throw new Error('Authentication token not found.');
      const baseUrl = process.env.NEXT_PUBLIC_API_URL;
      // Fetch leaves with status=REJECTED
      return await callApi('get', `${baseUrl}/leaves?page=${page}&limit=${limit}&status=REJECTED`, null, {
...
        return (
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(status)}`}>{status}</span>
        );
      },
    },
    {
...
  ];
    ...
    // ----------------------------------------------------------------------
    // View Leave Modal - MAX WIDTH ADJUSTED & STYLING IMPROVED
    // ----------------------------------------------------------------------
        ...
        return (
            <div className="w-full px-4 sm:px-6 lg:px-8">
                <div className="mx-auto mt-10 max-w-full xl:max-w-7xl rounded bg-white p-6 shadow relative">
                    {/* ^ Changed to max-w-full xl:max-w-7xl for big view */}
                    {isSubmittingStatus && (
...
  return (
    <div className="min-h-screen p-4 bg-gray-100"> {/* Changed padding to p-4 for consistency */}
      <Toaster position="top-center" reverseOrder={false} />
        ...
        <div className="mx-auto mt-10 max-w-full xl:max-w-7xl rounded bg-white p-6 shadow">
            ...
            <h3 className="mb-4 text-2xl font-semibold text-gray-700">Rejected Leave Records</h3> {/* Improved header size */}
                    ...
                    actions={(leave) => {
                        return (
                            <div className="relative">
                                <ActionButtons showView={true} onView={() => handleView(leave)} showUpdate={false} showDelete={false} />
                            </div>
                        );
                    }}
                />
