...
import Swal from 'sweetalert2'; // Kept for rejection reason modal
import withReactContent from 'sweetalert2-react-content'; // Kept for rejection reason modal
import { useQuery, useMutation } from '@tanstack/react-query'; // Removed useQueryClient: Not directly used for invalidation here
...
import { Toaster } from 'react-hot-toast'; // Typically, Toaster is in _app.tsx or layout.tsx, adding here for completeness if not global
...
function getStatusColor(status: string) {
    switch (status) {
        ...
        case 'N/A': // Added for clarity when status might be null
...
export default function RejectedLeavesPage() {
    ...
    // We don't need useQueryClient if we're just calling refetch directly
    // const queryClient = useQueryClient();

    const fetchLeaves = async ({ queryKey }: { queryKey: any[] }) => {
        const [_key, currentPage, itemsPerPage] = queryKey;
        if (!token) {
            // Use toast for this authentication error as well
            toast.error('Authentication token not found. Please log in.');
            throw new Error('Authentication token not found. Please log in.');
        }
        const baseUrl = process.env.NEXT_PUBLIC_API_URL;
        // Fetch only REJECTED leaves
        const responseData = await callApi('get', `${baseUrl}/leaves?page=${currentPage}&limit=${itemsPerPage}&status=REJECTED`, null, {
    ...
    const { data, isLoading, isError, refetch } = useQuery({
        queryKey: ['rejectedLeaves', currentPage, itemsPerPage], // Updated query key
        queryFn: fetchLeaves,
        ...
        if (viewedLeave?.id) {
            ...
            if (status === 'REJECTED') {
                ...
                if (responseReason === undefined || responseReason === false) {
                    toast('Action cancelled.', { icon: '👋' }); // Optional: show a small toast for cancellation
                    return;
                }
    ...
    if (isError) {
        return (
            <div className="flex h-full flex-col items-center justify-center text-red-500">
                <p>Failed to load rejected leave records. Please try again.</p>
    ...
    return (
        <div className="min-h-screen p-6 bg-gray-100">
            ...
            <div className="mb-6 flex items-center justify-between">
                {/* <h1 className="text-3xl font-bold">Rejected Leave Records</h1> */}
            </div>
            ...
            {/* Leave Table */}
            <div>
                <h3 className="mb-4 text-xl font-semibold">Rejected Leave Table</h3>
                <Table
                    columns={columns}
                    data={leaves}
